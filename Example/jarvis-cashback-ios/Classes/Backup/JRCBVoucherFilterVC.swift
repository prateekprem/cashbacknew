...
extension JRCBVoucherFilterVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if filterListViewModel.isShowMoreButton(indexPath: indexPath) {
            filterListViewModel.showMoreFilters(section: indexPath.section, completion: {[weak self] in
                self?.table.reloadSections(IndexSet(integer: indexPath.section), with: .automatic)
            })
            return
        }
