...
NSString * const oldBundleKey = @"com.one97.paytm";
NSString * const JRUUID = @"PAYTM_DEVICE_UUID";
NSString * const migrationDetails = @"migrationDetails";
...
+(NSString *)getBundleKey{
    if(JRUtilityManager.shared.moduleConfig.varient == JRVarientMall){
        return @"com.one97.mall";
    }else{
        return @"com.one97.paytm";
    }
}
...
//        NSString *bundleID = [[NSBundle mainBundle] objectForInfoDictionaryKey:bundleKey];
        sharedInstance = [[FXKeychain alloc] initWithService:[FXKeychain getBundleKey]
                                                 accessGroup:nil];
        // Condition to avoid migration of data in case of Mall App
        // Currently we are handling migration only for Paytm main App
        
        if(JRUtilityManager.shared.moduleConfig.varient == JRVarientPaytm){
            [(FXKeychain *)sharedInstance checkIfMigrationNeeded];
        }
    });
...
+ (instancetype)defaultKeychainSiri
{
    static id sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
// Method to check for Migration, It copies the Values from older Container to
// newer container of Keychain

- (void)checkIfMigrationNeeded {
    NSString * currentUUID = (NSString *)[self objectForKey: JRUUID];
   
    if (currentUUID.length == 0) {
        NSString *bundleID = [[NSBundle mainBundle] objectForInfoDictionaryKey:oldBundleKey];
        FXKeychain *previousContainer = [[FXKeychain alloc] initWithService:bundleID
                                                                accessGroup:nil];
        NSString * previousUUID = (NSString *)[previousContainer objectForKey: JRUUID];
        
        if (previousUUID.length > 0){
            [self setObject:previousUUID forKey:JRUUID];
            // Create a Key in Fxkeychain "MigrationVersion" which holds the version and timestamp  $ seperated so that we can identify that when Migration has happened if required
            NSString * timeStampValue = [NSString stringWithFormat:@"%f", (double)[[NSDate date] timeIntervalSince1970]];
            NSString *version = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
            // Safety check, just to avoid a case where version or timestamp is nil
            if (version.length > 0 && timeStampValue.length > 0) {
                [self setObject:[NSString stringWithFormat:@"%@$%@", version,timeStampValue] forKey:migrationDetails];
            }
        }
    }
}
