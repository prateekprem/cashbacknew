...
- (void)update
{
    ...
    for (NSInteger i = 0; i < tokenCount; i++)
    {
        ...
        if (newLine || wrapped || i == tokenCount - 1)
        {
            ...
            NSInteger lastIndex = (wrapped || newLine)? i>0 ? i - 1 : 0 : i;//App was crashing so condition is modified
...
- (void)setHTMLText:(NSString *)text
{
    if (![super.text isEqualToString:text])
    {
        super.text = text.removeHTMLTag;
        HTMLTokenizer *tokenizer = [[HTMLTokenizer alloc] initWithHTML:text];
        _layout.tokens = tokenizer.tokens;
        [self setNeedsDisplay];
    }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        NSString *hyperlinkUrl = [token.attributes.href trimWhitespace];
        NSString* webStringURL = [hyperlinkUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    {
        if (token && [_delegate respondsToSelector:@selector(HTMLLabel:tappedOutSideHyperlink:)])
        {
            CGRect frame = [_layout.frames[[_layout.tokens indexOfObject:token]] CGRectValue];
            [_delegate HTMLLabel:self tappedOutSideHyperlink:frame];
        }
    }
}
