...
NSString * const UAJSONSerializationErrorDomain = @"Serialization error occurred in Common utility";
...
+ (NSString *)stringWithObject:(id)jsonObject
    ...
    if (!acceptingFragments ||
        ...
        NSData *data = [NSJSONSerialization dataWithJSONObjectt:jsonObject
                                                       options:opt
                                                         error:error];
...
+ (nullable NSData *)dataWithJSONObjectt:(id)obj options:(NSJSONWritingOptions)opt error:(NSError **)error {
